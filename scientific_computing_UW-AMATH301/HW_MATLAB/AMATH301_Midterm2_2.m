...
%% Part c (2nd-order accuracy scheme for first derivative)
A4 = zeros(1,length(tspan));
A4(1) = (-3*A1(1,1)+4*A1(1,2)-A1(1,3))/(2*dt); % fwd-difference first point

for n=2:length(tspan)-1
   A4(n)=( A1(1,n+1) - A1(1,n-1) )/(2*dt);
end

% backward-difference for last point
A4(1001) = (3*A1(1,1001)-4*A1(1,1001-1)+A1(1,1001-2))/(2*dt);

% debug plot comparison between part a and part c
% plot(tspan,A4,'-o',tspan,A1(2,:),'-o')

%% Part c (4th-order accuracy scheme for first derivative)

A5 = zeros(1,length(tspan));
A5(1) = (-3*A1(1,1)+4*A1(1,2)-A1(1,3))/(2*dt); % fwd-difference 1st point
A5(2) = (-3*A1(1,2)+4*A1(1,3)-A1(1,4))/(2*dt); % fwd-difference 2nd point

for n=3:length(tspan)-2
   A5(n)=( -A1(1,n+2) + 8*A1(1,n+1) - 8*A1(1,n-1) + A1(1,n-2) )/(12*dt);
end

% bkwd-difference next-to-last point
A5(1001-1) = (3*A1(1,1001-1)-4*A1(1,1001-2)+A1(1,1001-3))/(2*dt);
% bkwd-difference last point
A5(1001) = (3*A1(1,1001)-4*A1(1,1001-1)+A1(1,1001-2))/(2*dt);
...
% debug plot comparison between 1st and 2nd derivatives
% plot(tspan,A5,'-o',tspan,A6,'-o')
% legend('\theta''','\theta''''')
...
% plot(tspan,A1(1,:),'-o',tspan,y23','-o')
% legend('\theta_{A1}','\theta_{A8}')

%% Part f (2nd-order Runge-kutta scheme)

A8 = zeros(2,length(tspan));
A8(:,1) = y0;
y1 = A8(1,:);
y2 = A8(2,:);
for n=1:length(tspan)-1
    y1(n+1) = y1(n) + dt*(y2(n)+dt*y2(n)/2);
    f21 = -gamma*y2(n) - sin(y1(n));
    f22 = -gamma*(y2(n) + dt*f21/2)...
            -sin(y1(n) + dt*f21/2);
    y2(n+1) = y2(n) + dt*f22;
    A8(:,n+1) = [y1(n+1); y2(n+1)];
end

% debug plot comparison between A8 and y23
% plot(tspan,A8(1,:),'-o',tspan,y23(:,1)','-o')
% legend('\theta_{A8}','\theta_{ode23}')