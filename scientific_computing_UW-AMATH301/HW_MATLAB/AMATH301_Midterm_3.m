...
for j=2:7
    ...
    a2(j) = (2 + a1(j-1) - a3(j-1))/5;
    a3(j) = (-a1(j-1) - a2(j-1))/4;

    A1 = [A1 [a1(j); a2(j); a3(j)]];

    % G-S A
    b1(j) = (1 - 2*b2(j-1) - b3(j-1))/8;
    b2(j) = (2 + b1(j) - b3(j-1))/5;
    b3(j) = (-b1(j) - b2(j))/4;
    ...
    c2(j) = (2 + c1(j-1) - c3(j-1))/5;
    c3(j) = -8*c1(j-1) - 2*c2(j-1);

    A3 = [A3 [c1(j); c2(j); c3(j)]];

    % G-S B
    d1(j) = 1 - d2(j-1) - 4*d3(j-1);
    d2(j) = (2 + d1(j) - d3(j-1))/5;
    d3(j) = -8*d1(j) - 2*d2(j);
...
[V,D] = eigs(A5,3,'SM'); % arrange eigenvalues/vectors largest mag
