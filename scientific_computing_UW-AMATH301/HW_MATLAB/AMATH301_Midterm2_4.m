...
%% Part f (2nd-order Runge-Kutta scheme, manual)

% This is my understanding of the 2nd order R-K scheme using the loop
% There is slight error between matrices generated with ode23 and this loop

A8 = zeros(2,length(tspan));
A8(:,1) = y0;

for n=1:length(tspan)-1
    % m = midpoint (intermediate estimate)
    m = [A8(1,n) + dt*A8(2,n)/2;
                 A8(2,n) + dt*(-gamma*A8(2,n) - sin(A8(1,n)))/2];
    A8(:,n+1) = [A8(1,n) + dt*m(2);
                 A8(2,n) + dt*(-gamma*m(2) - sin(m(1)))];
end

% Debug plot Comparing ode23() to manual 2nd-order Runge-Kutta scheme
% plot(tspan,y23','-o',tspan,A8,'-o')
% legend('\theta_{ODE23}','\theta''_{ODE23}','\theta_{A8}','\theta''_{A8}')